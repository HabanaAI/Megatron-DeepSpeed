...
* Run Mixtral 8x7b on 32 HPUs, Lazy mode, with FP8 precision, sequence length 32k:
  ```
  HL_USE_TRANSFORMER_ENGINE=1 \
  HL_FP8_FORMAT=hybrid \
  HL_USE_CACHE_FP8_WEIGHT=0 \
  HL_USE_CACHE_FP8_WEIGHT_FWD=0 \
  HL_FP8_AMAX_REDUCE=1 \
  HL_GRAD_ACCUM_DTYPE=bf16 \
  HL_FP8_AMAX_HISTORY_LEN=10 \
  HL_FP8_AMAX_RECOMPUTE_ALGO=max \
  HL_HOSTSFILE=$MEGATRON_DEEPSPEED_ROOT/scripts/hostsfile \
  HL_MOE_NUM_CAPACITY_BINS=10 \
  HL_NUM_NODES=4 \
  ...
  HL_ZERO_STAGE=1 \
  HL_CKP_ACT=1 \
  HL_USE_FUSED_SDPA_WITH_RECOMPUTE=0 \
  $MEGATRON_DEEPSPEED_ROOT/scripts/run_mixtral.sh
  ```
...
# Changelog
## 1.18.0
 - Added FP8 support for Mixtral.
 - Added 'mlp' recompute mode support and enabled it for Mixtral (HL_CKP_ACT=3) with FSDPA recompute.
