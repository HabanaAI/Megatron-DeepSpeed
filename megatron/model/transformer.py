...
class ParallelAttention(MegatronModule):
    ...
    def forward(self, hidden_states, attention_mask,
        ...
        if rotary_pos_emb is not None:
            q_pos_emb, k_pos_emb = rotary_pos_emb
            query_layer = apply_rotary_pos_emb(query_layer, q_pos_emb, self.cos_cached, self.sin_cached)
            key_layer = apply_rotary_pos_emb(key_layer, k_pos_emb, self.cos_cached, self.sin_cached)
            # TODO, can apply positional embedding to value_layer so it has
            # absolute positional embedding.
            # otherwise, only relative positional embedding takes effect
            # value_layer = apply_rotary_pos_emb(value_layer, k_pos_emb, self.cos_cached, self.sin_cached)
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        # config checkpoint mlp
        self.checkpoint_mlp = config.recompute_granularity == 'mlp'
    ...
    def _checkpointed_mlp_forward(self, *args):
        """
        Checkpointable MLP forward
        """
        return tensor_parallel.checkpoint(self.mlp.forward, False, *args)
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        if self.num_experts == 1:
            if self.checkpoint_mlp:
                mlp_output, mlp_bias = self._checkpointed_mlp_forward(layernorm_output)
            else:
                mlp_output, mlp_bias = self.mlp(layernorm_output)
        else:
            if self.checkpoint_mlp:
                mlp_output, moe_loss, _ = self._checkpointed_mlp_forward(layernorm_output)
            else:
