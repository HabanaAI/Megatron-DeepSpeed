...
def update_train_iters(args):
    ...
    if args.rampup_batch_size is None:
        ...
        if args.scheduler_batch_size is None:
            # sequence length and batch size scheduler. Use args.train_iters from user input.
            print_rank_0('Running with varying global_batch_size. Train iterations might be approximation.')
...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters and (args.train_tokens is None or \
        ...
        # Stop and checkpoint for sequence length sheduler
        if args.stop_after_num_samples and args.consumed_train_samples>=args.stop_after_num_samples:
            torch.distributed.barrier()
            print_datetime(f'Conusmed: {args.consumed_train_samples} samples and {args.consumed_train_tokens} tokens with args.seq_length={args.seq_length}')
            print_datetime(f'Stopping, after {iteration} iterations')
            return iteration
