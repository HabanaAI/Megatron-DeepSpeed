...
def validate_args(args, defaults={}):
    ...
    def verify_monotonic_increasing(param):
        # verify param input is montonic increasing
        for idx in range(len(param)-1):
            if param[idx+1] - param[idx] < 0:
                return False
        return True
    ...
    # varying batch size
    if args.batch_sched_samples and args.scheduler_batch_size:
        args.scheduler_batch_size = [int(sbs) for sbs in args.scheduler_batch_size]
        args.batch_sched_samples = [int(lbs) for lbs in args.batch_sched_samples]
        assert len(args.batch_sched_samples) == len(args.scheduler_batch_size)
        assert verify_monotonic_increasing(args.batch_sched_samples), 'batch_sched_samples needs to be montonic increasing'
        assert verify_monotonic_increasing(args.scheduler_batch_size), 'scheduler_batch_size needs to be monotic increasing'
    ...
    if get_accelerator().device_name() != "hpu":
        ...
        if args.use_fused_sdpa and args.recompute_granularity != "mlp":
            assert args.use_fused_sdpa_with_recompute is False, "Please use either " \
            "use_fused_sdpa_with_recompute or recompute_granularity but not both, " \
            "unless you are using recompute_granularity=mlp"
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument('--fp8-manual-mode',
                       type=lambda x: x.lower() in ['true', '1'],
                       default=True,
                       help='Enable manual setting when amax is aggregated.')
...
def _add_training_args(parser):
    ...
    group.add_argument('--scheduler-batch-size', type=str, nargs='+', default=None,
                       help='Batch size change as a function of consumed samples.'
                       '  --scheduler-batch-size <first batch size> '
                       '                      <second batch size> '
                       '                      <third batch size> '
                       '                      <etc> '
                       ' The transition between batch sizes will be according'
                       ' to batch-sched-samples which should have the same length'
                       'For example:'
                       '   --scheduler-batch-size 1024 2048 4096 \ '
                       '   --batch-sched-samples 0 100 1000'
                       'will start with global batch size 1024 after 100 samples'
                       ' global batch will increase to 2048 and after 1000 samples'
                       ' the batch size will increase to 4096 and will remain so until'
                       ' training ends. When scheduler-batch-size is not None, initial'
                       ' global batch size will be scheduler-batch-size[0].'
                       ' If scheduler-batch-size is None global batch'
                       ' size will be constant the whole training.'
                       ' If scheduler-batch-size is not None, it needs to be the initial value for batch_size,'
                       ' and batch-sched-samples[0] needs to be the current consumed samples or zero.')
    group.add_argument('--batch-sched-samples', type=str, nargs='+', default=None,
                       help='batch-sched-samples complements scheduler-batch-size.'
                       ' It determines the number of samples needed to transition'
                       ' global-batch-size in scheduler-batch-size.'
                       '  --batch-sched-samples <first batch size trasition in samples> '
                       '                      <second> '
                       '                      <third> '
                       '                      <etc> '
                       ' batch-sched-samples should have the same length as scheduler-batch-size'
                       'For example:'
                       '   --scheduler-batch-size 1024 2048 4096 \ '
                       '   --batch-sched-samples 0 100 1000'
                       'will start with global batch size 1024 after 100 samples'
                       ...
                       ' If scheduler-batch-size is not None, it needs to be the initial value for batch_size,'
                       ' and batch-sched-samples[0] needs to be the current consumed samples or zero.')
    group.add_argument('--stop-after-num-samples', type=int, default=None,
                       help='To train with several different sequence lengths'
                       ' run script several times and checkpoint every time.'
                       ' To stop script and checkpoint in order to change sequence'
                       ' length, set stop-after-num-samples.')
    group.add_argument('--recompute-activations', action='store_true',
                       help='recompute activation to allow for training '
                       'with larger models, sequences, and batch sizes.')
    group.add_argument('--recompute-granularity', type=str, default=None,
                       choices=['full', 'selective', 'mlp'],
...
def _add_activation_checkpoint_args(parser):
    ...
    group.add_argument('--pytorch-activation-checkpointing', action='store_true',
                       help='uses activation checkpointing from pytorch')
...
def _add_pytorch_args(parser):
    ...
    group.add_argument('--use-torch-compiled-autograd',
                       type=lambda x: x.lower() in ['true', '1'],
                       default=False,
                       help='Enable compiled autograd')

    group.add_argument('--compiled-autograd-fallback-mode',
                       type=str,
                       default="nothing",
                       choices=["nothing", "warn", "error"],
                       help='Optional compiled autograd fallback mode, \
                            default=\"nothing\"')
    return parser
